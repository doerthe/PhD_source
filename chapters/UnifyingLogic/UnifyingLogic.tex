...
On a syntactic level \owl is compatible with \rdf---there exists a representation of \owl in \rdf~\cite{owltordf}---but semantically, 
...
%Given that popularity other standards in the Semantic Web need to be based on this one just as envisioned by the stack.




% To exchange data the Semantic Web uses \rdf. \rdf can be expressed in several syntaxes, eg XML, Turtle or JSON-LD. 
%  In this thesis we are going to use Turtle syntax, since that
% is the syntax \rdf shares with Notation3. Knowledge is expressed in simple triples consisting of \emph{subject}, \emph{predicate} and \emph{object} ($s$, $p$, $o$). 
% Examples are displayed in 
% Listing~\ref{triple}. The triple in line 3 means \textit{``Jim is a cockatoo.''} 
% 
% Of course, this notion of \emph{meaning} has a formal definition as well:
% \rdf Semantics~\cite{RDFSemantics} defines simple interpretations. The functions $IS$ and $IL$ (together $I$) map the different elements of a triple into the domain
% of discourse which here consists of the set of 
% resources ($IR$) and the set of
% properties ($IP$). Properties are then assigned to elements of the powerset of all pairs of resources ($2^{IR\times IR}$) by an extra mapping $IEXT$.
% Triples are considered true if the pair consisting of the values of subject and object is element of the value assigned to the predicate, ie if $(I(s),I(o))\in IEXT(I(p))$. 
% \begin{lstlisting}[
%   float=t,
%   caption={Example triples in \rdf. },
%   label=triple]
...
% _:x a :Cockatoo.
% \end{lstlisting}
% \rdf furthermore offers the option to use existential quantification which can be expressed by using so-called blank nodes.  These can either be written 
% using brackets \texttt{[]}
% or using expressions starting with \texttt{\_:}. The triple in line 4 of Listing~\ref{triple} for example means that \textit{``Cockatoos exist.''} 
% For the interpretation of existentially quantified variables assignment functions $A$ 
% are used which map existential variables to the set of resources $IR$.  A triple containing a blank node is considered true if for this blank node such a mapping $A$ can 
% be found which makes 
% the triple true in combination 
% with the interpretation.
% 
% The format \rdf is broadly used in practice. The Linked Open Data Cloud \cite{LODCloud}
% \subsection{The relation to the base}
