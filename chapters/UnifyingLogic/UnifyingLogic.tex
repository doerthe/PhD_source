...
Description Logics in general and with them also OWL DL in particular are designed as a subset of First Order Logic which is known to be decidable. that means that 
given a set of formulas which are known to be valid in the logic and an arbitrary statement which can be expressed in the language of the logic, 
there always exists an effective method to
decide whether or not this statement is a logical consequence of the set of formulas. 
...
Our quest should therefore rather enable us to only reason with the information we need and accept that we can not know everything 
(which is a problem  not only the Semantics Web has to cope with but also something we all deal with in our daily life).
