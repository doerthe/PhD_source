...
the \emph{Unifying Logic}. Such a connection can, for example, only support the exchange of the results obtained by the different blocks 
such as the \emph{logic framework}
envisioned by
Kifer et al~\cite{rearch} or semantically combine Description Logics and rule-based reasoning in one logic as Knorr et all understand it \cite{knorr}. 
Regardless of the concrete vision, there are two conceptual differences between the building blocks which are commonly seen 
as challenging: the closed or open world assumption and decidability.
...
be derived, 
it is considered to be false (negation as failure). While such an assumption comes in handy when we have one single database which can be completely searched by applying a query, 
it is problematic 
...
For that reason \rdf, \rdf{}S and \owl are defined with the \emph{open world assumption}. In contrast to that, the query language SPARQL follows the closed world assumption---the
filter \texttt{NOT EXISTS} retrieves a positive result if no match can be found for a given pattern \cite[Section 8]{sparql}---and for rule languages that depends on the semantic 
chosen---SWRL~\cite{swrl} follows the open world assumption while RIF-PRD~\cite{rifprd} supports the closed world assumption. As Kifer et al~\cite{rearch} point out, the solution 
