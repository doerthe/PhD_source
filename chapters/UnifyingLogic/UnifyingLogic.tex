...
\section{The Logical Layers}
%The \emph{Unifying Logic} is the piece of the Semantic Web stack we are going to focus on in this thesis. As this logic is 
...
% and the next upper layer, \emph{Proof}, in more detail to then clearly define the requirements
% such a unifying logic needs to fulfil.

\subsection{Data Interchange}
To exchange data the Semantic Web uses \rdf. \rdf can be expressed in several syntaxes, namely XML, Turtle or JSON-LD. 
 In this thesis we are going to use Turtle syntax, since that
is the syntax \rdf shares with Notation3. Knowledge is expressed in simple triples consisting of \emph{subject}, \emph{predicate} and \emph{object}. Examples are displayed in 
...
\rdf Semantics~\cite{RDFSemantics} defines simple interpretations. The functions IS and IL map the different elements of a triple into the domain
...
Triples are considered true if the pair consisting of the values of subject and object $(IS(s),IS(o))\in IEXT(IS(p))$ is element of the 
\begin{lstlisting}[
  float=t,
...
For the interpretation of existentially quantified variables assignment functions A are used which map existential variables to the set of resources IR.
...
\subsection{Taxonomies and Ontologies}
