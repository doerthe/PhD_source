...
%  have a clearly defined semantics which enables us to unambiguously determine the meaning of a statement. 
 ...
 Another important aspect of a \ul %we have not discussed earlier
...
the concept of correctness is not even defined and formal properties like its expressivity compared to other frameworks %formal properties 
cannot 
...
Related to the problem we just described, there is another one: it is not clear yet, how \nthree logic behaves in comparison to other logics. 
...
In Section~\ref{req} we identified the four requirements on the \ul we want to focus on: (1) clear semantics, %the semantics of \nthree can be clearly defined by providing a model theory for the logic, 
...
% In the previous sections we already discussed which properties of \nthreelogic make it a promising candidate  for that role -- %to become the unifying logic for the Semantic Web -- 
...
% of formulas which makes it possible to state proofs in it -- but we also discussed various open questions connected to the four requirements 
% mentioned above. We now go through all requirements to extract the different research questions which need to be answered.
...
% 
% Notation3 Logic is able to fulfil the requirements 
...
% There are many concepts about whose meaning the sources are not entirely clear. The %main difference in interpretations we could identify is on 
% concept causing most discrepancies
...
\hyp{\hypertarget{h2}{Hypothesis 1:}}{Existing interpretations differ in their understanding of implicit quantification and this difference can be 
...
To answer this research question, we need to take a closer look to the semantics of \rdf \cite{RDFSemantics}. This semantics is defined following a model theoretic approach.
In order  to stay consistent which these definitions, we need to follow the same apporach when defining the semantics of \nthree 
% This is formally defined by a model theory which
% -- in order to guarantee compatibility -- needs to be compatible with the model theory we define for \nthree.
%which is formally defined 
%We thus formalise the semantics of \nthree following 
according to the interpretations of Cwm and EYE. %choose one of the possible interpretations of \nthreelogic 
...
%Regarding Requirement~4, the support of proofs, 
%we already know that 
There exists a vocabulary in \nthree which was created to express proofs, the SWAP-proof vocabulary~\cite{Proof}. % makes it possible to express proofs in \nthree and the reasoners Cwm and EYE produce such proofs.
...
support the layer of \emph{Trust} but also the layer of \emph{Applications}. %Among the many ways of applying proofs we consider one example in more detail:
We illustrate that on the idea of using \nthree proofs for adaptive planning.
%In this thesis we are not only interested in the layer of proof to establish \emph{Trust} but also in the relation of that layer to \emph{Applications}. % we ask our last research question:
% 
% \hyp{Research Question 7:}{Are there applications for \nthree proofs which go beyond the establishment of \emph{Trust}?} 
% 
%We therefore furthermore investigate 
