...
The query language SPARQL treats \rdf as a data structure in which can be searched for particular patterns. In that sense, SPARQL only makes use of the syntax of \rdf and 
...
While this notation is syntactically compatible with \rdf, SPIN conflicts with \rdf's model theory: To express SPARQL's query variables SPIN uses blank nodes. The problem here is
...
For the ontology level the Semantic Web Stack proposes the Web Ontology Language (OWL)~\cite{owlold} which has evolved further since the creation 
of the Semantic Web Stack to OWL~2~\cite{owl}.
...
We want to have a unifying logic which connects the functionalities. We want \rdf as a base. We want to be able to choose how the reasoning deals with owl given that owl full is 
undecidable. 
