% We then map the interpretations of Cwm and EYE to that core logic.
...
\section{N3 Syntax}\label{n3synsec}
...
%the concept of 
% passing information through syntax trees can be helpful for our problem at hand: the formalisation of \nthree's implicit quantification.
% of formulas 
% containing implicit universal quantification 
% can help use to compute their interpretations.

% which we are going to
% introduce in the following section. In this section we define the syntax of \nthree and illustrate t
%Before we can do that, we first formaly define the syntax of \nthree.
% 
% To make the connection between \nthree syntax and its possible interpretation we use of attribute grammars~\cite{attributegrammar,ag2}. Such grammars can be used to pass information through 
% the syntax tree of a formula 
...
attribute grammars. % provide a formalism to describe . 
This concept will be introduced in the following section.


\section{Attribute Grammars}\label{ag}
...
\section{From N3 syntax to N3 Core Logic}
\begin{figure}\centering
\small
\begin{tabular}{llll}
\hline
...
\subsection{Existentials}\label{exsec}
% tree without labels
% \begin{figure}
% \begin{center}
% \begin{tikzpicture}
...
\subsection{Universals in Cwm}\label{unicwm}
% \begin{figure}
...
Before we define attribute $s$ below which takes care of this behaviour, we consider one additional example to fully understand the consequences of the exception.
%As a consequence of that behaviour, adding another 
...
% This is done with 
%To take the behaviour we define a
...
\subsection{Generation of the translation}\label{all}
\begin{figure*}
\begin{minipage}{\textwidth}
...
\end{tabular}
\end{center}   \normalsize
\caption{Attribute rules for constructing the translation of a formula into \nthree Core Logic according to Cwm ($m_c$) and according to EYE ($m_e$).\label{trans}}
\end{minipage}
\end{figure*}

In order to obtain the different translations from \nthree to \nthree Core Logic, 
one last step is needed: the \nthree Core Logic formulas need to be generated. %Before defining attributes to perform this task, we first define  auxiliary functions. 
